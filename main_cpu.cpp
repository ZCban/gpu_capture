...
// D2D rimosso completamente — niente d2d1.lib
...
// D2D rimosso: pD2DContext, pD2DTargetBitmap, pLastValidBitmap eliminati
...
// pre-allocato una volta sola
py::array_t<float> g_result;
...
bool initialize_capture(UINT width, UINT height, bool use_rgba, bool use_zoom)
{
    ...
    // D2D factory/device/context rimossi completamente

    // texture destinazione crop — solo SHADER_RESOURCE, non serve RENDER_TARGET
    ...
    desc.BindFlags = D3D11_BIND_SHADER_RESOURCE; // rimosso RENDER_TARGET
    ...
    // pre-alloca numpy array una volta sola
    g_result = py::array_t<float>({ 1, 3, (int)cropHeight, (int)cropWidth });
...
py::object get_frame_image()
{
    ...
    // D2D eliminato — crop diretto con CopySubresourceRegion
    D3D11_BOX cropBox = {};
